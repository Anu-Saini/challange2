...
function generatePrompt() {
    const pwdLength = prompt("Password should be atleast 8 characters and no longer than 128 characters.");
    const pwdCase = prompt("Password should contain a combination uppercase and lowercase letters.");
    const pwdNumber = prompt("Password should contain atleast 1 number.");
    const pwdspecialChar = prompt("Password should contain atleast 1 special characters.");
    return { length: pwdLength, case: pwdCase, number: pwdNumber, specialChar: pwdspecialChar };
}
...
function generateRandomPassword(pool, pwd) {
    ...
    while (pwd.length < minLength)
        generateRandomPassword(pool, pwd);
    return pwd;
